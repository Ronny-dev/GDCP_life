...
public class TodayActivity extends Activity{
    ...
    private void initListView(todayData_javabean moneyData) {
        ListView listView = (ListView) findViewById(R.id.card_activity_today_listview);
        listView.setAdapter(new TodayListViewAdapter(TodayActivity.this, moneyData));
    }


    private void initView(todayData_javabean moneyData) {
        ...
        int moneySum = 0;
        for (todayData_javabean.RowsBean rowsBean : moneyData.getRows()) {
            if(String.valueOf(rowsBean.getTRANAMT()).contains("-")){
                moneySum += rowsBean.getTRANAMT();
            }
        }
        tv_money.setContent(String.valueOf(moneySum).replace("-", ""));
    }
    ...
    private void initData() {
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                bean = cardClient.getTodayTransact(today, today, no);
    ...
    private void solveData() {
        ...
        for(int i = 0;i<bean.getRows().size();i++){
            String time = bean.getRows().get(i).getOCCTIME();
            int hour = Integer.parseInt(time.substring(time.indexOf(":") - 2, time.lastIndexOf(":") - 3));
            if(!String.valueOf(bean.getRows().get(i).getTRANAMT()).contains("-")){
                continue;
            }
            String money = String.valueOf(bean.getRows().get(i).getTRANAMT()).replace("-","");
            if(hour>=6&&hour<9){
                dataMoney[0] += (float) Double.parseDouble(money);
            }else if(hour>=9&&hour<12){
                dataMoney[1] += (float) Double.parseDouble(money);
            }else if(hour>=12&&hour<15){
                dataMoney[2] += (float) Double.parseDouble(money);
            }else if(hour>=15&&hour<18){
                dataMoney[3] += (float) Double.parseDouble(money);
            }else if(hour>=18&&hour<21){
                dataMoney[4] += (float) Double.parseDouble(money);
            }else if(hour>=21&&hour<24){
                dataMoney[5] += (float) Double.parseDouble(money);
            }
        }
        score = dataMoney;
        mPointValues.clear();
