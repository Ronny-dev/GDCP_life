package com.example.ronny_xie.gdcp.adapter;

import java.io.File;
import java.util.List;
import com.example.ronny_xie.gdcp.activity.ChatPage;
import com.example.ronny_xie.gdcp.activity.ShowBigImage;
...
import com.example.ronny_xie.gdcp.util.TimeUtil;
import com.example.ronny_xie.gdcp.util.ToastUtil;
import com.example.ronny_xie.gdcp.view.RTPullListView;
...
public class ChatMessageAdapter extends BaseAdapter {
	...
	public ChatMessageAdapter(ChatPage activity, List<GotyeMessage> messageList, RTPullListView pullListView) {
	...
	@Override
	public int getCount() {
		if(messageList.size()!=0&&open){
			System.out.println(messageList.size()+1);
			pullListView.setSelection(messageList.size()+1);
			open = false;
		}
		...
		if (getDirect(message) == MESSAGE_DIRECT_SEND) {
			...
			case GotyeMessageStatusSendingFailed: // 发送失败
				...
				holder.staus_iv.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						iscunzai = false;
						new AlertDialog.Builder(chatPage)
						.setTitle("提示")
						.setMessage("是否重新发送")
