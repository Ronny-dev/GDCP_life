...
public class jwxskb_activity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				if (msg.what == 2) {
					jwxskb_adapter adapter = new jwxskb_adapter(BeanData,jwxskb_activity.this);
	...
	private void setButtonListener() {
		btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				ProgressDialogUtil.showProgress(jwxskb_activity.this, "请稍后...");
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				xskcBfromHTML = ConnInterface.getXSKCBfromHTML(
						fragment_jw.httpClient, fragment_jw.values,
						login.getUser(jwxskb_activity.this)[0],
						login.getUser(jwxskb_activity.this)[1], tem);
