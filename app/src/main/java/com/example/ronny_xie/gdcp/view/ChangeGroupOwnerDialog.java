...
public class ChangeGroupOwnerDialog extends Dialog implements
	...
	public void onClick(View arg0) {
		...
		case R.id.back:
			...
			break;
		case R.id.ok:
			ProgressDialogUtil.showProgress(getContext(), "正在转让并退出群...");
	...
	class MemberAdapter extends BaseAdapter {
		...
		private void setMemberIcon(ImageView iconView, GotyeUser member) {
			if (member.getIcon() != null && member.getIcon().getPath() != null
					&& !"".equals(member.getIcon().getPath())) {
				Bitmap bmp = BitmapUtil.getBitmap(member.getIcon().getPath());
				if (bmp != null) {
					iconView.setImageBitmap(bmp);
					return;
				}
				GotyeAPI.getInstance().downloadMedia(member.getIcon());
			}
