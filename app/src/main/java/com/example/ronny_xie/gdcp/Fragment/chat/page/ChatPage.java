package com.example.ronny_xie.gdcp.Fragment.chat.page;
...
//                         showPopupWindow2(pressToVoice);
    ...
    private void initRightTopBtnListener() {
        ImageView btn_RightTop = (ImageView) findViewById(R.id.right_menu);
        btn_RightTop.setBackgroundResource(R.drawable.to_info_normal);
        btn_RightTop.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (onRealTimeTalkFrom >= 0) {
                    ToastUtil.show(getApplication(), "正在实时语音..");
                    return;
                }
                // 语音消息判断
                if (makingVoiceMessage) {
                    ToastUtil.show(getApplication(), "正在录音语音..");
                    return;
                }
...
//                showTalkView();
    ...
    public void hideKeyboard() {
        ...
        pullListView.setSelection(adapter.getCount() + 1);
        System.out.println("这里是隐藏输入法：" + adapter.getCount() + 1);
    }
