package com.example.ronny_xie.gdcp.Fragment.WeatherActivity;
...
import com.example.ronny_xie.gdcp.Fragment.WeatherActivity.WeatherJavaBean.adapter_gridView_popwindows;
import com.example.ronny_xie.gdcp.Fragment.WeatherActivity.WeatherJavaBean.adapter_listview_popwindow3;
import com.example.ronny_xie.gdcp.Fragment.WeatherActivity.WeatherJavaBean.adapter_weather_main;
import com.example.ronny_xie.gdcp.Fragment.WeatherActivity.WeatherJavaBean.adapter_weather_weibo;
import com.example.ronny_xie.gdcp.Fragment.WeatherActivity.WeatherJavaBean.javabean_weather_lifeNotic;
import com.example.ronny_xie.gdcp.Fragment.WeatherActivity.WeatherJavaBean.javabean_weather_main;
import com.example.ronny_xie.gdcp.Fragment.WeatherActivity.WeatherJavaBean.javabean_weather_main_pad;
import com.example.ronny_xie.gdcp.Fragment.WeatherActivity.WeatherJavaBean.javabean_weather_popwindow1;
import com.example.ronny_xie.gdcp.Fragment.WeatherActivity.WeatherJavaBean.javabean_weather_weibo;
import com.example.ronny_xie.gdcp.Fragment.WeatherActivity.WeatherJavaBean.javabean_weather_worn;
import com.example.ronny_xie.gdcp.Fragment.WeatherActivity.WeatherJavaBean.weather_util;
...
import com.example.ronny_xie.gdcp.Fragment.WeatherActivity.weather.presenter.WeatherPresenter;
import com.example.ronny_xie.gdcp.Fragment.WeatherActivity.weather.util.LocationUtils;
import com.example.ronny_xie.gdcp.Fragment.WeatherActivity.weather.widget.WeatherChartView;
...
public class fragment_weather extends Activity {
    ...
    private static final int CAIYUNTITLESUCCESS = 2003;
    private static final int CAIYUNDESCSUCCESS = 2004;
    ...
    private TextView tvCaiYunTitle;
    private TextView tvCaiYunDesc;
    ...
    private void initCaiYunMap() {
        linearLayout = (LinearLayout) findViewById(R.id.fragment_main_linearlayout_caiyun);
        if (ContextCompat.checkSelfPermission(this,
            ...
            if(LocationUtils.latitude != 0 && LocationUtils.longitude != 0)
                startInitCaiYunData(LocationUtils.latitude, LocationUtils.longitude);
            else
                linearLayout.setVisibility(View.GONE);
        }
    }

    private void startInitCaiYunData(final double latitude, final double longitude) {
        tvCaiYunTitle = (TextView) findViewById(R.id.fragment_weather_caiyun_title);
        tvCaiYunDesc = (TextView) findViewById(R.id.fragment_weather_caiyun_desc);
        new Thread(new Runnable() {
            @Override
            public void run() {
                final String address = WeatherPresenter.loadLocationAddress(latitude, longitude);
                if(address != null){
                    handler.sendMessage(handler.obtainMessage(CAIYUNTITLESUCCESS, address));
                }
            }
        }).start();
        new Thread(new Runnable() {
            @Override
            public void run() {
                final WeatherForecastInfo json = WeatherPresenter.loadForecastWeather(latitude, longitude);
                if(json != null && !json.getResult().getMinutely().getDescription().equals("")){
                    handler.sendMessage(handler.obtainMessage(CAIYUNDESCSUCCESS, json.getResult().getMinutely().getDescription()));
    ...
    private void initChat(WeatherForecastInfo json) {
        mWeatherChartView = (WeatherChartView) findViewById(R.id.wc_two_hour);
        mWeatherChartView.setLineColor(getResources().getColor(R.color.default_blue));
        WeatherForecastInfo.ResultBean.MinutelyBean minutely = json.getResult().getMinutely();
...
//        //更新两小时降雨量曲线
    ...
    private void initHandler() {
        handler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                if (msg.what == CAIYUNTITLESUCCESS){
                    String title = (String) msg.obj;
                    tvCaiYunTitle.setText(title);
                }
                if (msg.what == CAIYUNDESCSUCCESS){
                    String desc = (String) msg.obj;
                    tvCaiYunDesc.setText(desc);
                }
            }
        };
    }
    ...
    private void setBackgroundFromBing() {
       String requsetBingPic = "http://guolin.tech/api/bing_pic";
        URIUtil.sendOkHttpRequest(requsetBingPic, new Callback() {
            @Override
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Glide.with(fragment_weather.this).load(bingPic).into(imageBackground);
                    }
                });
            }
        });
